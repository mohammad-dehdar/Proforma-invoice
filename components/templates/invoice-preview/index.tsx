...
export const InvoicePreview = ({ setShowPreview, handlePrint }: InvoicePreviewProps) => {
    ...
    return (
        <>
            <div className="min-h-screen bg-linear-to-br from-slate-50 via-stone-50 to-slate-100 py-8 px-4">
                {/* Action Buttons */}
                <div className="no-print mb-8 flex gap-4 justify-center">
                    <button
                        onClick={() => setShowPreview(false)}
                        className="bg-slate-800 hover:bg-slate-900 text-white px-8 py-3 rounded-lg font-medium transition-all duration-200 shadow-sm hover:shadow-md"
                    >
                        بازگشت به ویرایش
                    </button>
                    <button
                        onClick={handlePrint}
                        className="bg-emerald-700 hover:bg-emerald-800 text-white px-8 py-3 rounded-lg flex items-center gap-3 font-medium transition-all duration-200 shadow-sm hover:shadow-md"
                    >
                        <Download size={20} />
                        چاپ / ذخیره PDF
                    </button>
                ...
                <div
                    ref={printRef}
                    className="print-container bg-white rounded-2xl shadow-xl max-w-5xl mx-auto overflow-hidden"
                    dir="rtl"
                >
                    {/* Header with Elegant Design */}
                    <div className="relative bg-linear-to-l from-slate-900 via-slate-800 to-slate-900 px-12 py-10">
                        <div className="flex justify-between items-start">
                            <div className="text-white">
                                <h1 className="text-5xl font-bold mb-2 tracking-tight">اتمیفای</h1>
                                <p className="text-slate-300 text-sm font-light tracking-wide">فرم پیش فاکتور خرید محصول</p>
                            </div>
                    ...
                    <div className="px-12 py-10">
                        ...
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <div>
                                <h2 className="text-2xl font-bold text-slate-900">فاکتور فروش</h2>
                                <p className="text-slate-500 text-sm">جزئیات تراکنش مالی</p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-10">
                            <div className="bg-slate-50 border border-slate-200 p-5 rounded-xl">
                                <p className="text-slate-500 text-sm mb-2 font-medium">شماره فاکتور</p>
                                <p className="text-xl font-bold text-slate-900">{invoice.number}</p>
                            </div>
                            <div className="bg-slate-50 border border-slate-200 p-5 rounded-xl">
                                <p className="text-slate-500 text-sm mb-2 font-medium">تاریخ صدور</p>
                        ...
                        <div className="grid md:grid-cols-2 gap-6 mb-10">
                            {/* Customer Info */}
                            <div className="bg-linear-to-br from-blue-50 to-indigo-50 border border-blue-100 p-6 rounded-xl">
                                <div className="flex items-center gap-3 mb-5 pb-4 border-b border-blue-200">
                                    <div className="bg-blue-600 p-2 rounded-lg">
                                        <User className="text-white" size={20} />
                                    </div>
                                    <h3 className="text-lg font-bold text-slate-900">مشخصات خریدار</h3>
                                </div>
                                <div className="space-y-3">
                                    <div>
                                        <p className="text-slate-500 text-xs mb-1 font-medium">نام و نام خانوادگی</p>
                                        <p className="text-slate-900 font-semibold">{invoice.customer.name}</p>
                                    </div>
                                    {invoice.customer.company && (
                                        <div>
                                            <p className="text-slate-500 text-xs mb-1 font-medium">نام شرکت</p>
                                            <p className="text-slate-900 font-semibold">{invoice.customer.company}</p>
                                        </div>
                                    )}
                                    {invoice.customer.phone && (
                                        <div>
                                            <p className="text-slate-500 text-xs mb-1 font-medium">شماره تماس</p>
                                            <p className="text-slate-900 font-semibold">{invoice.customer.phone}</p>
                                        </div>
                                    )}
                                    {invoice.customer.address && (
                                        <div>
                                            <p className="text-slate-500 text-xs mb-1 font-medium">آدرس</p>
                                            <p className="text-slate-900 font-semibold leading-relaxed">{invoice.customer.address}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            {/* Payment Info */}
                            <div className="bg-linear-to-br from-emerald-50 to-teal-50 border border-emerald-100 p-6 rounded-xl">
                                <div className="flex items-center gap-3 mb-5 pb-4 border-b border-emerald-200">
                                    <div className="bg-emerald-600 p-2 rounded-lg">
                                        <CreditCard className="text-white" size={20} />
                                    </div>
                                    <h3 className="text-lg font-bold text-slate-900">اطلاعات پرداخت</h3>
                                </div>
                                <div className="space-y-3">
                                    <div>
                                        <p className="text-slate-500 text-xs mb-1 font-medium">شماره کارت</p>
                                        <p className="text-slate-900 font-semibold font-mono tracking-wider">
                                            {invoice.paymentInfo.cardNumber}
                                        </p>
                                    </div>
                                    <div>
                                        <p className="text-slate-500 text-xs mb-1 font-medium">صاحب کارت</p>
                                        <p className="text-slate-900 font-semibold">{invoice.paymentInfo.cardHolderName}</p>
                                    </div>
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-slate-900">
                                        <th className="p-4 text-right text-white font-semibold text-sm">ردیف</th>
                                        <th className="p-4 text-right text-white font-semibold text-sm">شرح خدمات</th>
                                        <th className="p-4 text-center text-white font-semibold text-sm">تعداد</th>
                                        <th className="p-4 text-right text-white font-semibold text-sm">قیمت واحد</th>
                                        <th className="p-4 text-right text-white font-semibold text-sm">مبلغ کل</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {invoice.services.map((service: Service, index: number) => (
                                        <tr key={service.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                            <td className="p-4 text-slate-600 font-medium">{formatNumber(index + 1)}</td>
                                            <td className="p-4 text-slate-900 font-medium">{service.description}</td>
                                            <td className="p-4 text-center text-slate-600 font-medium">{formatNumber(service.quantity)}</td>
                                            <td className="p-4 text-slate-600 font-medium">{formatPrice(service.price)} تومان</td>
                        ...
                        {/* Calculations */}
                        <div className="bg-slate-50 border border-slate-200 p-6 rounded-xl mb-6 space-y-3">
                            <div className="flex justify-between items-center">
                                <span className="text-slate-600 font-medium">جمع کل خدمات:</span>
                                <span className="text-slate-900 font-bold text-lg">{formatPrice(calculateSubtotal())} تومان</span>
                            </div>
                            {invoice.discount && invoice.discount > 0 && (
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-600">تخفیف ({invoice.discount}%):</span>
                                    <span className="text-red-600 font-bold">-{formatPrice(calculateDiscount())} تومان</span>
                                </div>
                            )}
                            {invoice.tax && invoice.tax > 0 && (
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-600">مالیات ({invoice.tax}%):</span>
                                    <span className="text-yellow-600 font-bold">+{formatPrice(calculateTax())} تومان</span>
                                </div>
                            )}
                        </div>

                        {/* Total Amount */}
                        <div className="bg-linear-to-l from-slate-900 via-slate-800 to-slate-900 p-8 rounded-xl mb-8">
                            <div className="flex justify-between items-center">
                                <div>
                                    <p className="text-slate-300 text-sm mb-1">جمع کل قابل پرداخت</p>
                                    <p className="text-white text-3xl font-bold">{formatPrice(calculateTotal())} تومان</p>
                                </div>
                            </div>
                        </div>
                        ...
                        {invoice.notes && invoice.notes.trim() && (
                            <div className="bg-amber-50 border-r-4 border-amber-400 p-6 rounded-xl mb-8">
                                <h3 className="text-amber-900 font-bold mb-2">یادداشت:</h3>
                                <p className="text-amber-800 leading-relaxed whitespace-pre-line">{invoice.notes}</p>
                            </div>
                        )}

                        {/* Footer */}
                        <div className="pt-8 border-t-2 border-slate-200 text-center">
                            <p className="text-slate-900 text-lg font-semibold mb-3">با تشکر از اعتماد شما</p>
                            <div className="flex justify-center items-center gap-6 text-slate-600 text-sm flex-wrap">
